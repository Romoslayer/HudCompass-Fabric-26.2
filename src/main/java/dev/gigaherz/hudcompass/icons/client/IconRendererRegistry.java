...
public class IconRendererRegistry
{
    ...
    private static final BasicIconRenderer BASIC_ICON_RENDERER = registerRenderer(HudCompass.BASIC_ICON_SERIALIZER,
            new BasicIconRenderer());

    public static final BasicIconData MISSING_ICON = new BasicIconData(HudCompass.location("unknown"));

    private static final IIconRenderer MISSING_ICON_RENDERER = (data, player, textureManager, matrixStack, x, y, alpha) ->
            BASIC_ICON_RENDERER.renderIcon(MISSING_ICON, player, textureManager, matrixStack, x, y, alpha);
    ...
    public static void renderIcon(IIconData<?> data, Player player, TextureManager textureManager, GuiGraphicsExtractor graphics, int x, int y, int alpha)
    {
        IIconRenderer renderer = REGISTRY.computeIfAbsent(data.getSerializer(), (key) -> {
            LOGGER.warn("Missing icon renderer for {}", key.name());
